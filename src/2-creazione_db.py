from sqlalchemy import create_engine, Column, Integer, String, TIMESTAMP, Float, Enum, ForeignKey, PrimaryKeyConstraint, UniqueConstraint
...
class IntentISA95Link(Base):
    '''
    allo stato attuale è posibile avere un intento senza classificazione isa.
    non sono sicuro questa cosa vada bene
    '''
...
class EntityISA95Link(Base):
    '''
    allo stato attuale è posibile avere un entita senza classificazione isa.
    non sono sicuro questa cosa vada bene
    '''
...
class IntentMatch(Base):
    ...
    '''
    occorre ancora capire come gestire il caso
        intent_a_id=x intent_b_id=y ed
        intent_a_id=y intent_b_id=x
    '''
    id = Column(Integer, primary_key=True, autoincrement=True)
...
class EntityMatch(Base):
    ...
    '''
    occorre ancora capire come gestire il caso
        entity_a_id=x entity_b_id=y ed
        entity_b_id=y entity_a_id=x
    '''
